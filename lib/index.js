...
const { Vouchers } = require('./v0/vouchers')
...
class Tillhub extends EventEmitter {
   ...
   *
   * @param {Object} options options object
   * @return {Templates} templates instance
   */
  templates (options) {
    if (!this.http) throw errors.generate(errors.constants.UNINSTANTIATED_HTTP_CLIENT)
   ...
   *
   * @param {Object} options options object
   * @return {Vouchers} vouchers instance
   */
  vouchers (options) {
    if (!this.http) throw errors.generate(errors.constants.UNINSTANTIATED_HTTP_CLIENT)
    return new Vouchers({ user: this.user, base: this.options.base, ...options }, this.http)
  }
}
