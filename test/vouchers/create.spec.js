...
test('v0: Vouchers: can create voucher', function (t) {
  ...
  if (!process.env.SYSTEM_TEST) {
    ...
    test.nock('https://api.tillhub.com').post('/api/v0/vouchers/anotheruuid').reply(200, {
      'status': 200,
      'msg': 'Voucher created.',
      'count': 0,
      'results': [
  ...
  th.init((err, client, authInstance) => {
    ...
    vouchers.create({ format: 'XXXX XXXX', format_type: 'numeric' }, (err, data, metadata) => {
      t.error(err)
