...
test('v1: Templates: can create template', function (t) {
  ...
  if (!process.env.SYSTEM_TEST) {
    ...
    test.nock('https://api.tillhub.com').post('/api/v1/templates/anotheruuid').reply(200, {
      'status': 200,
  ...
  th.init((err, authResponse, client, authInstance) => {
    ...
    templates.create({ type: 'invoice_v1', options: { title: 'INVOICE' } }, (err, data, metadata) => {
      t.error(err)
